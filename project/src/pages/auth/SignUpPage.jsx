...
import { ArrowLeft, Briefcase, UserCheck as UserSearch, X } from 'lucide-react';
            ...
            <CardContent className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <motion.div whileHover={{ scale: 1.05 }} className="w-full">
                  <Button
                    onClick={() => { setOnboardingType('candidate'); setShowOnboarding(true); }}
                ...
                <motion.div whileHover={{ scale: 1.05 }} className="w-full">
                  <Button
                    onClick={() => { setOnboardingType('employer'); setShowOnboarding(true); }}
      ...
      {showOnboarding && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="relative w-full max-w-lg mx-auto">
            <button
              className="absolute top-4 right-4 z-10 bg-white/30 hover:bg-white/60 rounded-full p-2 shadow-xl backdrop-blur"
            ...
            <HirlyOnboarding
              userTypeFromRoute={onboardingType}
              onComplete={() => { setShowOnboarding(false); navigate('/hub'); }}
            />
          </div>
        </div>
      )}
